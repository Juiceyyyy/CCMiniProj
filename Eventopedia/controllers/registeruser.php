	...
	if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		redirectTo(localhost('register.php'));
			...
			if (count($errors) == 0) {
				try {
					if ($values['type'] == "user") {
						$sql = "insert into users(`name`, `DOB`, `gender`, `contact`, `email`, `password`) values (:name, :dob, :gender, :contact, :email, :password)";
					} else {
						$sql = "insert into admin( `name`, `dob`, `gender`, `contact`, `email`, `password`) values (:name, :dob, :gender, :contact, :email, :password)";
					}
					$stmt = $conn->prepare($sql);
					$stmt->bindParam(':name', $values['name']);
					$stmt->bindParam(':dob', $values['dob']);
